...
3. **Seed data is unimplemented.** `database/seeds/UgandaLocaleSeeder.php`
   calls per-table seeders (`RegionTableSeeder`, `DistrictTableSeeder`, etc.)
   that don't exist anywhere in the repo, and `database/factories/ModelFactory.php`
   has no factories defined. The actual Uganda administrative data still
   needs to be sourced and loaded.
